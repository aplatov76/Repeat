...
public class AddProductGate
{
  ...
  public void updateSize() { ConnectDB mysql = new ConnectDB();
    mysql.updateSizeAfterAdded();
  }
  
  public void updateTable() { ConnectDB mysql = new ConnectDB();
    int ns = expected.size();
    LinkedHashMap<Integer, Integer> map = new LinkedHashMap();
    int n = Repeat.admprod.size();
    for (int i = 0; i < n; i++) { map.put(Integer.valueOf(((AdminPane)Repeat.admprod.get(i)).getId()), Integer.valueOf(i));
    }
    for (int i = 0; i < ns; i++) {
      ...
      Repeat.admprod.set(index, new AdminPane(index, isnow.getId(), isnow.getName(), isnow.getShortname(), isnow.getGroup(), isnow.getHelf(), size_total, isnow.getPrice(), 0));
    }
    expected.clear();
  }
