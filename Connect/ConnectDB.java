...
public class ConnectDB
{
  ...
  public void loadUser(ObservableList<Collection.Users> user)
  {
    ...
    try {
      ...
      while (rs.next()) { user.add(new Collection.Users(rs.getString(2), rs.getString(3), rs.getString(1), rs.getInt(5)));
      }
  ...
  public void loadProductId(ObservableList<Integer> data)
  {
    try
    {
      ...
      while (rs.next()) { data.add(Integer.valueOf(rs.getInt(1)));
      }
      rs.close();
      stmt.close();
      connection.close();
  ...
  public void loadGroupTable(ObservableList<Collection.GroupProduct> data)
  {
    ...
    try {
      ...
      while (rs.next()) { data.add(new Collection.GroupProduct(n++, rs.getInt(2), rs.getString(1)));
      }
      rs.close();
      stmt.close();
      connection.close();
  ...
  public void loadGroupList(ObservableList<String> data)
  {
    ...
    try {
      ...
      while (rs.next()) { data.add(rs.getInt(2) + " " + rs.getString(1));
      }
      rs.close();
      stmt.close();
      connection.close();
  ...
  public void loadRules(ObservableList<String> data)
  {
    try
    {
      ...
      while (rs.next()) { data.add(rs.getInt(1) + "  " + rs.getString(2));
      }
      rs.close();
      stmt.close();
      connection.close();
  ...
  public void setLog(AdminPane p, AdminPane prev, int type, String user)
  {
    try
    {
      ...
      if (type == 1) { stmt.executeUpdate("INSERT INTO `revision_log_product`(`id`,`id_operation`,`id_product`,`name_product`,`group_product`,`ostatok`,`price`) VALUES ('0','" + maxindex + "'," + "'" + prev.getId() + "'," + "'" + prev.getShortname() + "'," + "'" + prev.getGroup() + "'," + "'" + prev.getSize() + "'," + "'" + prev.getPrice() + "');");
      }
  ...
  public void setLog(ListCustProduct p, int type, String user)
  {
    try
    {
      ...
      if (type == 1) { stmt.executeUpdate("INSERT INTO `revision_log_product`(`id`,`id_operation`,`id_product`,`name_product`,`group_product`,`ostatok`,`price`) VALUES ('0','" + maxindex + "'," + "'" + p.getId() + "'," + "'" + p.getName() + "'," + "'" + p.getGroup() + "'," + "'0'," + "'" + p.getPrice() + "');");
      }
  ...
  public int setProductAdmin(String name, String sname, int gr, int h, int size, double p)
  {
    int b = 0;
    try
    {
  ...
  public double getStartSum(String user_name, boolean full_short)
  {
    ...
    try {
      ...
      Statement stmt = connection.createStatement();
      ResultSet rs = stmt.executeQuery(query);
      if (rs.next()) { start_sum = rs.getDouble(1);
      }
      rs.close();
      connection.close();
    }
  ...
  public String getGroup(int id)
  {
    ...
    try {
      ...
      Statement stmt = connection.createStatement();
      ResultSet rs = stmt.executeQuery(query);
      if (rs.next()) { group = rs.getString(1);
      }
      rs.close();
      connection.close();
    }
  ...
  public void getSessionBecap(ObservableList<Registration> prod, String user_name)
  {
    try {
      ...
      if (rs.next()) metallrasxod += rs.getDouble(1);
      c -= metallrasxod;
      Collection.Cassa.cassa = c;
  ...
  public void getOtchet(ObservableList<Repot> prod, String usr_name)
  {
    ...
    try
    {
      ...
      Statement stmt = connection.createStatement();
      ResultSet rs = stmt.executeQuery(query);
      while (rs.next()) { prod.add(new Repot(rs.getString(2), Integer.toString(rs.getInt(1)), Integer.toString(rs.getInt(4)), Double.toString(price(rs.getString(2))), rs.getDouble(5)));
      }
      query = "SELECT SUM(sum) AS sum_with FROM `registration` WHERE `data` = '" + sysd + "' and `user` = '" + usr_name + "';";
      rs = stmt.executeQuery(query);
      double sum_with = 0.0D;
      if (rs.next()) sum_with = rs.getDouble(1);
      ...
      if (contracts_summa_paid != 0.0D) { prod.add(new Repot("", "", "", "Итого: ", contracts_summa_paid));
      }
      ...
      if (sum_with_metall != 0.0D) { prod.add(new Repot("", "", "", "Итого: ", sum_with_metall));
      }
      ...
      if (rs.next()) { cassa_start_short = rs.getDouble(1);
      }
  ...
  public void getOtchet(ObservableList<Repot> prod)
  {
    ...
    try
    {
      ...
      while (rs.next()) { prod.add(new Repot(rs.getString(2), Integer.toString(rs.getInt(1)), Integer.toString(rs.getInt(4)), Double.toString(rs.getDouble(3)), rs.getDouble(5)));
      }
      ...
      int n = key.length - 1;
      if (n > 1) for (int i = 0; i < n; i++) { prod.add(new Repot("", "", "", i + 1 + ". " + key[i].toString(), ((Double)map.get(key[i])).doubleValue()));
        }
  ...
  public ObservableList<String> getGrapicsDate(String a, String b)
  {
    ...
    try
    {
      ...
      while (rs.next()) category.add(rs.getDate(1).toString());
      rs.close();
      stmt.close();
      connection.close();
    }
  ...
  public void getProductForStatus(ObservableList<Prices> prod, int status)
  {
    try
    {
      ...
      Connection connection = DriverManager.getConnection(url, properties);
      Statement stmt = connection.createStatement();
      if (status == 0) actual = " WHERE `actual_status` = 0;";
      if (status == 1) actual = " WHERE `actual_status` = 1;";
      if (status == 2) actual = " WHERE `actual_status` = 2;";
      String query = "SELECT * FROM `prais`" + actual;
      ResultSet rs = stmt.executeQuery(query);
      int e = 0;
      while (rs.next()) { prod.add(new Prices(rs.getInt(1), e++, rs.getString(3), rs.getInt(4), rs.getInt(7), rs.getDouble(6), rs.getInt(8)));
      }
      rs.close();
      stmt.close();
  ...
  public void getProductForStatusAdmin(ObservableList<AdminPane> prod, int status)
  {
    try {
      ...
      while (rs.next()) prod.add(new AdminPane(index++, rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getInt(5), rs.getInt(7), rs.getDouble(6), rs.getInt(8)));
      rs.close();
      stmt.close();
  ...
  public void getGroupPrices(ObservableList<Prices> prod, int gr, int s)
  {
    try
    {
      ...
      int e = 0;
      while (rs.next()) { prod.add(new Prices(rs.getInt(1), e++, rs.getString(3), idgroup, rs.getInt(7), rs.getDouble(6), rs.getInt(8)));
      }
      rs.close();
      stmt.close();
  ...
  public void getGroupProductAdmin(ObservableList<AdminPane> prod, int gr, int s)
  {
    try
    {
      ...
      ResultSet rs = stmt.executeQuery(query);
      while (rs.next()) {
        prod.add(new AdminPane(index, rs.getInt(1), rs.getString(2), rs.getString(3), idgroup, rs.getInt(5), rs.getInt(7), rs.getDouble(6), rs.getInt(8)));
        index++;
      }
  ...
  public void getCustomerProductGroup(ObservableList<ListCustProduct> prod, String gr)
  {
    try
    {
      ...
      ResultSet rs = stmt.executeQuery(query);
      while (rs.next()) { prod.add(new ListCustProduct(count++, rs.getInt(1), rs.getString(2), rs.getInt(4), rs.getDouble(3)));
      }
      rs.close();
      stmt.close();
  ...
  public void updatePrice(int id, String name, String sname, int group, int helf, double price, int size, int actual_status)
  {
    try {
      Class.forName("com.mysql.jdbc.Driver");
  ...
  public void getHistory(String a1, String a2, ObservableList<Collection.History> prod)
  {
    try
    {
      ...
      while (rs.next()) { prod.add(new Collection.History(rs.getInt(1), rs.getDate(2).toString(), rs.getString(5), rs.getInt(6), rs.getInt(4), rs.getInt(7), rs.getDouble(8), rs.getDouble(9), rs.getString(10)));
      }
      rs.close();
      stmt.close();
      connection.close();
  ...
  public Log_view getRevision_product(int id, int index, String time) {
    ...
    try
    {
      ...
      if (rs.next()) { log1 = new Log_view(index, time, id, rs.getInt(3), rs.getString(4), rs.getInt(5), rs.getInt(6), rs.getDouble(7), 1, "");
      }
      rs.close();
      stmt.close();
      connection.close();
  ...
  public int setNewUser(String name, String pass, int rule, boolean use_key, String key)
  {
    ...
    try
    {
      ...
      if (rs.next()) { idbreak = rs.getInt(1);
      }
      stmt.close();
      connection.close();
    }
  ...
  public ObservableList<Integer> getHelf(int id)
  {
    ...
    try {
      ...
      for (int i = 1; i <= max; i++) { helf.add(Integer.valueOf(i));
      }
    }
    catch (ClassNotFoundException|SQLException e) {}
  ...
  public void getContractsList(int i, int j, ObservableList<Collection.contract> prod)
  {
    try
    {
      ...
      Class.forName("com.mysql.jdbc.Driver");
      String query = null;
      if (i == 1) query = "SELECT * FROM `contracts` WHERE `status` ='1' and `type_contract` = '" + j + "';";
      if (i == 0) query = "SELECT * FROM `contracts` WHERE `status` ='0' and `type_contract` = '" + j + "';";
      if (i == 2) { query = "SELECT * FROM `contracts` WHERE `type_contract` = '" + j + "';";
      }
      Connection connection = DriverManager.getConnection(url, properties);
      Statement stmt = connection.createStatement();
      ResultSet rs = stmt.executeQuery(query);
      
      while (rs.next()) { prod.add(new Collection.contract(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getDate(5).toString(), rs.getString(6), rs.getString(7), rs.getDate(8).toString(), rs.getDate(9).toString(), rs.getDouble(10), rs.getDouble(11), rs.getDouble(12), rs.getBoolean(13), rs.getString(14)));
      }
  ...
  public void loadProductCustomer(ObservableList<ListCustProduct> data)
  {
    try
    {
      ...
      int count = 0;
      while (rs.next()) { data.add(new ListCustProduct(count++, rs.getInt(1), rs.getString(2), rs.getInt(4), rs.getDouble(3)));
      }
  ...
  public int setAddProductSizeIsnow(String name_product, int id_product, int size_second, String user, boolean sttus) {
    ...
    try
    {
      ...
      ResultSet rs = stmt.executeQuery(query);
      if (rs.next()) {
        if (rs.getInt(1) == 0) { stmt.executeUpdate("INSERT INTO `procurement_product_hist` (`id`, `date`, `name_product`, `id_product`, `size_second`, `user`,`status`) VALUES ('0', '" + sysdate + "', '" + name_product + "', '" + id_product + "', '" + size_second + "', '" + user + "','" + st + "');");
        } else {
        ...
        if (rs.next()) { indexend = rs.getInt(1);
        }
      }
  ...
  public boolean validateDeletGroup(int id)
  {
    ...
    try
    {
      ...
      ResultSet rs = stmt.executeQuery(query);
      if ((rs.next()) && (rs.getInt(1) != 0)) r = false;
      stmt.close();
  ...
  public void getBecapMetall(ObservableList<MetallList> data)
  {
    try
    {
      ...
      ResultSet rs = stmt.executeQuery(query);
      int start = 1;
      while (rs.next()) { data.add(new MetallList(start++, rs.getInt(3), rs.getString(4), rs.getDouble(5), rs.getDouble(7), rs.getInt(6)));
      }
      rs.close();
      stmt.close();
      connection.close();
  ...
  public void getHistoryMetall(ObservableList<HistoryMetallOtchet> prod, String d1, String d2, int typegroup, int typesort)
  {
    ...
    try
    {
      ...
      if (typegroup != -1) { selectgroup = "`code` = " + typegroup + " AND";
      }
      ...
      if (typesort == 0) query = "SELECT id,data,code,name,size,summa,price FROM `metall_reg` WHERE " + selectgroup + " `data` >= '" + d1 + "' and `data` <= '" + d2 + "';";
      if (typesort == 1) query = "SELECT id,data,code,name,SUM(size) as Size_sum,SUM(summa) AS Summa_sum,ROUND(avg(price),3) FROM `metall_reg` WHERE " + selectgroup + " `data` >= '" + d1 + "' and `data` <= '" + d2 + "' GROUP BY `data`;";
      if (typesort == 2) { query = "SELECT id,data,code,name,SUM(size) as Size_sum,SUM(summa) AS Summa_sum,ROUND(avg(price),3) FROM `metall_reg` WHERE  `data` >= '" + d1 + "' and `data` <= '" + d2 + "' GROUP BY `code`;";
      }
      
      if (typesort == 3) { query = "SELECT `id`,`data`,`code`,`name`,SUM(`size`) as Size_sum,SUM(`summa`) AS Summa_sum,ROUND(avg(`price`),3) FROM `metall_reg` WHERE  `data` >= '" + d1 + "' and `data` <= '" + d2 + "' GROUP BY `data`,`code`;";
      }
      Connection connection = DriverManager.getConnection(url, properties);
      Statement stmt = connection.createStatement();
      ResultSet rs = stmt.executeQuery(query);
